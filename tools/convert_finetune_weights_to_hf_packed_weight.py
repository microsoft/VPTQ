...
def pack_index(indice: torch.Tensor,
    ...
    if res_indice is not None:
        merged_indice = ((res_indice.view(index_dtype).to(torch.uint64).view(
            torch.int64) << index_bits) | indice.view(
            index_dtype).to(torch.uint64).view(torch.int64))
    else:
        merged_indice = indice.view(index_dtype).to(
            torch.uint64).view(torch.int64)
    ...
    paded_bits = (
        32 - out.reshape(*merged_indice.shape[:-1], -1).shape[-1] % 32) % 32
    out = torch.nn.functional.pad(out, (0, paded_bits),
                                  value=0, mode='constant',).reshape(
        *merged_indice.shape[:-1], -1, 32)
        ...
        if 'qlinear' in str(type(sub_mod)):
            ...
            if sub_mod.indices.dtype == torch.int64:
                sub_mod.indices.data = dtype_convert(
                    ...
                    sub_mod.indices.data.dtype, to_dtype, as_type)
            else:
                sub_mod.indices.data = dtype_convert(
                    sub_mod.indices.data,
                    from_dtype, to_dtype, as_type)
            ...
            if sub_mod.perm.dtype == torch.int64:
                sub_mod.perm.data = dtype_convert(
                    sub_mod.perm.data,
                    sub_mod.perm.data.dtype, to_dtype, as_type)
            else:
                sub_mod.perm.data = dtype_convert(
                    sub_mod.perm.data,
                    from_dtype, to_dtype, as_type)
...
def eval_ppl(qmodel, config):
    ...
    for dataset in datasets:
        dataloader, testloader = get_loaders(
            dataset, model=config.model_args.model_name,
            seqlen=qmodel.seqlen)
        print(dataset)
...
if __name__ == "__main__":
    ...
    print(
        f'load model from {args.load_model}, from_type: {from_type}, to_type: {to_type}')
    ...
    def load_model_shards(state_path):
        ...
        shard_paths = glob.glob(
            osp.join(state_path, 'model-*-of-*.safetensors'))
    ...
    if qmodel.seqlen != 4096 and ('llama2' in config.model_args.model_name.lower() or 'llama-2' in config.model_args.model_name.lower()):
    ...
    model = convert_idx_dtype(qmodel, from_type,
                              to_type, as_type)
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        f'{args.base_model}', legacy=False)
