...
class QuantLinear(nn.Module):
        ...
        if self.vector_quant_dim == "in":
            ...
            if self.is_indice_packed is True:
                ...
                self.indices = Parameter(
                    torch.empty(
                        (self.num_codebooks, self.num_indices, packed_groupsize), dtype=index_dtype, device=device
                    ),
                    requires_grad=False,
                )
            else:
                ...
                if self.indices_as_float:
                    self.indices = Parameter(
                        torch.empty(
                            (self.num_codebooks, self.num_indices, self.group_size), dtype=torch.float16, device=device
                        ),
                        requires_grad=False,
                    )
                else:
                    self.indices = Parameter(
                        torch.empty(
                            (self.num_codebooks, self.num_indices, self.group_size), dtype=torch.int16, device=device
                        ),
                        requires_grad=False,
                    )
    ...
    def fast_gemv(self, x):
        ...
        outlier_centroids = (
            ...
            if hasattr(self, "outlier_centroids")
            else None
        )
        if self.indices.dtype == torch.int:
