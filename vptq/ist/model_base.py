...
def make_quant_linear(module, quant_conf, name='', target_layer=None):
    for module_name, sub_module in tqdm(module.named_modules(), total=len(list(module.named_modules())),
                    desc="Replacing linear layers..."):
        if module_name in quant_conf:
            layer_conf = quant_conf[module_name]
            new_module = target_layer(**layer_conf, indices_as_fp16=False, enable_proxy_error=False, dtype=sub_module.weight.dtype)
            #tmp = sub_module
...
def pack_model(model, quantizers, args):
    ...
    quant_config = {"group_size":args.vector_len, "outliers":args.npercent, "residual":True}
    make_quant_linear(model, quantizers, quant_config, target_layer=target_layer, device="cpu")
    ...
    for name in tqdm(qlayers, desc='Packing weights....'):
        ...
        qlayers[name].pack(invperm, q_indice, q_indice_residual, centroids, residual_centroids,outliers, outliers_indice)
