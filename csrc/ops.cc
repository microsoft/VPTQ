...
torch::Tensor dequant(const torch::Tensor &q_indice,
                    ...
                    int groupsize, int in_features, int out_features) {
  ...
  if (q_indice_residual.has_value()) { 
    TORCH_CHECK(q_indice.size(0) == centroids.size(0) && 
      centroids.sizes() == residual_centroids.value().sizes(), "size must be same");
  ...
  output = lauch_deqantize_outliers_cuda_packkernel(out_f_x_in_f, q_indice, centroids, q_indice_residual, residual_centroids, 
